module.exports = {
  ...
  deploy: {
    staging: {
      ...
      "post-deploy":
        "cd /root/api/dev/designflix-api/current && npm ci --no-audit --no-fund --unsafe-perm || npm install --no-audit --no-fund --unsafe-perm && NODE_ENV=development node scripts/setEnv.js development && npx sequelize-cli db:migrate && pm2 reload ecosystem.development.config.js --only designflix-api-dev && pm2 save --force && pm2 list",
